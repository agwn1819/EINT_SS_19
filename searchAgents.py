...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        # Wenn alle Ecken besucht dann Ziel
        isGoal = (state[1] == [])
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # state[0] -> aktuelle Position
            x, y = state[0]
            dx, dy = Actions.directionToVector(action)
            nextX, nextY = int(x + dx), int(y + dy)

            # Wenn nicht Wand beruerht, dann naechste Position nextX,nextY
            if not self.walls[nextX][nextY]:
                nextState = (nextX, nextY)
                cost = 1

                # Wenn naechste Position eine der Ecken ist, dann Position aus Liste von Ecke entfenrnen
                if nextState in state[1]:
                    # Kopieren der Liste von Ecken
                    newCorner = list(state[1])
                    newCorner.remove(nextState)
                    # Neue Position + neue Liste von Ecken in Nachfolger einfuegen in Form von (Position, Aktion,
                    # Kosten)
                    successors.append(((nextState, newCorner), action, cost))
                else:
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def __init__(self, fn='depthFirstSearch', prob='PositionSearchProblem', heuristic='nullHeuristic'):
        super().__init__(fn='depthFirstSearch', prob='PositionSearchProblem', heuristic='nullHeuristic')
        self.actionIndex = 0
        self.actions = []

    def registerInitialState(self, state, nextPathSegment=None):
        currentState = state
        while currentState.getFood().count() > 0:
            self.findPathToClosestDot(currentState)
